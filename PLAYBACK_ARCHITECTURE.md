...
| (10) Audiobook export hardening | Done | Export settings now expose audiobook speed, progress is driven by authoritative SSE `completedCount` snapshots, and MP3 download uses a same-origin Next proxy route. |
...
- `AudiobookExportModal` now exposes audiobook speed in the export settings
  alongside native model speed. For `1x` downloads the proxy streams the worker
  MP3 unchanged; for other speeds it applies an ffmpeg `atempo` transform during
  download so the exported MP3 itself changes speed.
...
- The new Next audio route resolves/authenticates the playback session, mints the
  ...
  headers for raw downloads, optionally transcodes tempo for non-`1x` audiobook
  speed, and sets `Content-Disposition`.

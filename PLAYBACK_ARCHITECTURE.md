...
1. Audiobook export artifacts are user/document-scoped. Artifact and metadata
   ...
   - Export operation invalidation on cache reset matches opKey scope only;
     a same-document/settings collision from another user is benign because
     artifacts are user-scoped objects and export resolve is artifact-first.
2. Export retention is enforced. The worker owns a
   `POST /v1/maintenance/exports/expire` endpoint that scans
   `account_exports_v1/` and `tts_playback_exports_v1/`, reads metadata
   sidecars in batches, and deletes artifact directories (metadata + ZIP/audio
   + manifest) whose `createdAt` is past the retention window. Metadata is
   written only on completion, so in-flight preparations are never swept, and
   the schema floors `maxAgeMs` at one hour. The Next scheduled task
   `expire-export-artifacts` is a short trigger (default daily, 7-day
   retention) per the Step 17 maintenance-route contract; expired exports are
   simply regenerated by the next resolve/create request.

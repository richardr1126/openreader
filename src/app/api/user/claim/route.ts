...
): Promise<{ documents: number; preferences: number; progress: number; documentSettings: number; folders: number; onboarding: number }> {
  const [[docCount], [preferencesCount], [progressCount], [settingsCount], [folderCount], [onboardingCount]] =
    await Promise.all([

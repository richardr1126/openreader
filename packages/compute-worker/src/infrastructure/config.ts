...
export type ComputeOperationKind = 'pdf_layout' | 'tts_playback' | 'tts_playback_plan' | 'tts_playback_export';
...
export function getWorkerClientWaitTimeoutMs(kind: ComputeOperationKind): number {
  const config = getComputeTimeoutConfig();
  const timeoutMs = kind === 'pdf_layout' || kind === 'tts_playback_export'
    ? config.pdfTimeoutMs

...
describe('server-state architecture', () => {
  ...
  test('drives TTS playback through worker-owned progressive streams', () => {
    ...
    expect(context).toContain('startOrdinal: startSegment.ordinal');
    ...
    expect(streamTimelineRoute).toContain('buildSeekLayout');
    expect(streamTimelineRoute).toContain("throw new Error('TTS playback timeline requires a canonical plan artifact')");
    expect(streamTimelineRoute).toContain('ordinal: slot.segmentIndex');
    expect(streamTimelineRoute).toContain('segmentKey: slot.segmentKey');
    expect(streamTimelineRoute).toContain('durationMs: slot.durationMs');
    expect(streamTimelineRoute).toContain('locator: slot.locator');

---
title: Docker Quick Start
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

## Prerequisites

- A recent Docker version installed
- A TTS API server that OpenReader can reach:
  - [Kokoro-FastAPI](./configure/tts-provider-guides/kokoro-fastapi)
  - [KittenTTS-FastAPI](./configure/tts-provider-guides/kitten-tts-fastapi)
  - [Orpheus-FastAPI](./configure/tts-provider-guides/orpheus-fastapi)
  - [Replicate](./configure/tts-provider-guides/replicate)
  - [DeepInfra](./configure/tts-provider-guides/deepinfra)
  - [OpenAI](./configure/tts-provider-guides/openai)
  - [Other OpenAI-compatible providers](./configure/tts-provider-guides/other)

:::warning SeaweedFS Compatibility Note (April 16, 2026)
OpenReader currently pins embedded SeaweedFS to `4.18` in CI and Docker builds.
`4.19` introduced intermittent `InternalError` responses on S3 `PutObject` in our upload flow.
:::

## Published images

- App server: `ghcr.io/richardr1126/openreader:latest`
- Compute worker (Optional): `ghcr.io/richardr1126/openreader-compute-worker:latest`
- Legacy app alias: `ghcr.io/richardr1126/openreader-webui:latest`

## 1. Start the Docker container

Generate two secrets once and keep them for future container recreations:

```bash
openssl rand -base64 32 # AUTH_SECRET
openssl rand -base64 32 # TTS_PLAYBACK_TOKEN_SECRET
```

Replace the placeholders below with those values. Do not generate new values during a routine image
upgrade: `AUTH_SECRET` protects sessions and stored provider credentials, while
`TTS_PLAYBACK_TOKEN_SECRET` signs worker-owned playback URLs.

<Tabs groupId="docker-start-mode">
<TabItem value="localhost" label="Localhost" default>

Persistent storage, embedded SeaweedFS `weed mini`, required auth, optional library mount:

```bash
docker run --name openreader \
  --restart unless-stopped \
  -p 3003:3003 \
  -p 8081:8081 \
  -v openreader_docstore:/app/docstore \
  -e API_BASE=http://host.docker.internal:8880/v1 \
  -e API_MODEL_NAME=kokoro \
  -e BASE_URL=http://localhost:3003 \
  -e AUTH_SECRET=replace-with-generated-auth-secret \
  -e COMPUTE_WORKER_HOST=0.0.0.0 \
  -e COMPUTE_WORKER_PUBLIC_URL=http://localhost:8081 \
  -e TTS_PLAYBACK_TOKEN_SECRET=replace-with-generated-playback-secret \
  -e ADMIN_EMAILS=you@example.com \
  ghcr.io/richardr1126/openreader:latest
```

What this command enables:

- `-p 3003:3003`: exposes the OpenReader web app/API.
- `-p 8081:8081`: exposes signed, worker-owned playback audio to the browser. Worker control routes still require the private worker token generated by the embedded runtime.
- `-v openreader_docstore:/app/docstore`: persists SQLite metadata, SeaweedFS blob data, and migration/runtime state.
- `-e API_BASE=...` / optional `-e API_KEY=...` / `-e API_MODEL_NAME=...`: **first-boot seed only.** On the first container start, these are stored in a `default-openai` admin shared provider in the DB (key encrypted at rest when provided). `API_MODEL_NAME` defaults to `kokoro`. After that, the running app no longer reads them — manage the provider from **Settings → Admin → Shared providers**. See [Admin Panel](./configure/admin-panel).
- `COMPUTE_WORKER_HOST`, `COMPUTE_WORKER_PUBLIC_URL`, and `TTS_PLAYBACK_TOKEN_SECRET` enable background generation and browser playback through the embedded worker.
- `-e BASE_URL=...` and `-e AUTH_SECRET=...`: required for auth/session startup.
- `-e ADMIN_EMAILS=...`: (optional, requires auth) comma-separated emails auto-promoted to admin. Admins see the **Admin** tab in Settings.

</TabItem>
<TabItem value="local-network" label="LAN Host">

Use this when the app should be reachable from other devices on your LAN:

```bash
docker run --name openreader \
  --restart unless-stopped \
  -p 3003:3003 \
  -p 8081:8081 \
  -v openreader_docstore:/app/docstore \
  -e API_BASE=http://host.docker.internal:8880/v1 \
  -e API_MODEL_NAME=kokoro \
  -e BASE_URL=http://<YOUR_LAN_IP>:3003 \
  -e AUTH_SECRET=replace-with-generated-auth-secret \
  -e COMPUTE_WORKER_HOST=0.0.0.0 \
  -e COMPUTE_WORKER_PUBLIC_URL=http://<YOUR_LAN_IP>:8081 \
  -e TTS_PLAYBACK_TOKEN_SECRET=replace-with-generated-playback-secret \
  -e AUTH_TRUSTED_ORIGINS=http://localhost:3003,http://127.0.0.1:3003 \
  -e USE_ANONYMOUS_AUTH_SESSIONS=true \
  -e ADMIN_EMAILS=you@example.com \
  ghcr.io/richardr1126/openreader:latest
```

Replace `YOUR_LAN_IP` with the Docker host IP address on your local network to allow access from other devices.

What this command enables:

- LAN access from phones/tablets/other computers via `http://<YOUR_LAN_IP>:3003`.
- `BASE_URL` points auth/session cookies and callbacks at your LAN URL.
- `COMPUTE_WORKER_PUBLIC_URL` points signed playback audio at a URL the other devices can reach.
- `AUTH_TRUSTED_ORIGINS` allows localhost loopback origins in addition to your primary LAN origin.
- `USE_ANONYMOUS_AUTH_SESSIONS=true` allows guest sessions while auth is enabled.
- `API_BASE` seeds the default TTS endpoint into the admin-managed `default-openai` shared provider on first boot. Edit it from **Settings → Admin → Shared providers** after that.
- `API_KEY` optionally seeds the default provider's key (encrypted at rest). Omit it for an upstream that does not require authentication.
- `API_MODEL_NAME` seeds the provider's default model; it defaults to `kokoro` when omitted or blank.
- `ADMIN_EMAILS=...` (optional) auto-promotes the listed email(s) to admin so they can manage shared providers and site feature flags from the UI.
- `openreader_docstore` volume keeps data persistent across restarts.

</TabItem>
<TabItem value="minimal" label="Minimal">

Auth required, embedded storage ephemeral, no library import:

```bash
docker run --name openreader \
  --restart unless-stopped \
  -p 3003:3003 \
  -p 8081:8081 \
  -e BASE_URL=http://localhost:3003 \
  -e AUTH_SECRET=replace-with-generated-auth-secret \
  -e COMPUTE_WORKER_HOST=0.0.0.0 \
  -e COMPUTE_WORKER_PUBLIC_URL=http://localhost:8081 \
  -e TTS_PLAYBACK_TOKEN_SECRET=replace-with-generated-playback-secret \
  ghcr.io/richardr1126/openreader:latest
```

What this command enables:

- Fast startup with only the required auth env vars.
- No persistent volume (`/app/docstore` stays container-local), so data is ephemeral unless you add a mount.
- The app still requires `BASE_URL` + `AUTH_SECRET`, and playback requires the worker URL/signing variables, even in minimal mode.
- No TTS provider preset by default. Configure `API_BASE`, optional `API_KEY`, and `API_MODEL_NAME` on first boot if you want a seeded shared provider, or run auth+admin mode and manage providers from the admin panel.

</TabItem>
</Tabs>

:::tip Quick Tips
- Set `API_BASE` on first boot to a TTS endpoint the container can reach (`host.docker.internal` works for host-local services), and set `API_MODEL_NAME` to the model that endpoint accepts. After first boot, manage providers in **Settings → Admin → Shared providers**.
- `BASE_URL` and `AUTH_SECRET` are required. The admin panel requires auth.
- Set `ADMIN_EMAILS` to your email if you want the **Admin** tab in Settings.
- TTS credentials are configured only through **Settings → Admin → Shared providers**.
- Use a `/app/docstore` mount if you want data to survive container/image replacement.
- Startup automatically runs DB migrations and the v4 legacy storage decommission via the shared entrypoint.
- Scheduled maintenance tasks run in-process and can be managed from **Settings → Admin → Scheduled tasks**; Docker/self-hosted deployments do not need `CRON_SECRET`.
:::

:::info Linux host TTS
Docker Desktop resolves `host.docker.internal` automatically. On Linux, add
`--add-host=host.docker.internal:host-gateway` when the TTS server runs on the Docker host.
The embedded topology proxies storage through the app, so port `8333` does not need browser access.
:::

## 2. Configure settings in the app UI

Visit [http://localhost:3003](http://localhost:3003) after startup.

- If you set `ADMIN_EMAILS`, sign in with that email and open **Settings → Admin** to manage shared TTS providers and site feature flags for all users.
- Per-user: select an enabled shared TTS provider/model in **Settings → TTS Provider**.
- Select the model voice from the voice dropdown.

## 3. Upgrade from v4.4 to v5

Legacy image compatibility: `ghcr.io/richardr1126/openreader-webui:latest` remains available as an alias.
For external compute mode image details, see [Compute Worker (NATS JetStream)](./deploy/compute-worker).

1. Back up the volume that contains `/app/docstore`.
2. Stop and remove the old container, but keep its volume.
3. Pull the v5 image.
4. Recreate the container using the same `/app/docstore` volume and existing `AUTH_SECRET`, plus
   the v5 playback settings shown above.

```bash
docker stop openreader
docker rm openreader
docker pull ghcr.io/richardr1126/openreader:latest
# Re-run your docker run command from step 1 with the same named volume and secrets.
```

The image entrypoint automatically applies all pending SQLite/Postgres migrations before serving
traffic. Once storage is ready, it also performs the v4 decommission by deleting the retired
`tts_segments_v1/`, `tts_segments_v2/`, and `audiobooks_v1/` object prefixes. The operation is
idempotent, so no separate command or one-off migration container is needed for normal Docker
upgrades. Check startup logs for both migration completion messages before smoke testing.

:::tip
If you use a mounted volume for `/app/docstore`, documents, accounts, settings, and current playback
artifacts remain after image updates. Only the retired v4 cache/export prefixes are decommissioned.
:::

:::info Related Docs
- [Environment Variables](./reference/environment-variables)
- [Auth](./configure/auth)
- [Admin Panel](./configure/admin-panel)
- [Database](./configure/database)
- [Object / Blob Storage](./configure/object-blob-storage)
- [Migrations](./configure/migrations)
:::
